...
![Project Canvas](docs/images/project_canvas.png)
...
![Project Canvas](docs/images/Hypothesis1.png)
...
![Project Canvas](docs/images/H1.2.png)
...
![Project Canvas](docs/images/H1.1.png)
...
![Project Canvas](docs/images/Hypothesis2.png)
...
![Project Canvas](docs/images/H2.1.png)
...
![Project Canvas](docs/images/H2.3.png)
...
![Project Canvas](docs/images/H2.2.png)
...
![Project Canvas](docs/images/Hypothesis3.png)
...
![Project Canvas](docs/images/H3.1.png)
...
![Project Canvas](docs/images/H3.2.png)
...
![Project Canvas](docs/images/H3.3.png)
...
![Project Canvas](docs/images/Hypothesis4.png)
...
![Project Canvas](docs/images/H4.1.png)
...
![Project Canvas](docs/images/H4.2.png)
...
![Project Canvas](docs/images/H4.3.png)
...
![Project Canvas](docs/images/H4.4.png)
...
* [fact_bookings](data/fact_bookings.csv) 
* [dim_room_inventory](data/dim_room_inventory.csv) 
* [dim_channels](data/dim_channels.csv) 
* [dim_rate_codes](data/dim_rate_codes.csv) 
* [fact_marketing_spend](data/fact_marketing_spend.csv) 
* [dim_calendar](data/dim_calendar.csv) 

# Synthetic Data Engineering Prompts
```
...
## 1. วิเคราะห์จำนวนการจองแยกตาม Booking Channel
* จุดประสงค์ เพื่อดูว่าช่องทางใดสร้าง volume สูงสุด และโรงแรมพึ่งพาช่องทางใดมากที่สุด
* เหตุผลที่เลือกใช้ : Bar Chart เหมาะสำหรับเปรียบเทียบจำนวนข้อมูลระหว่างหลายหมวดหมู่ เช่น Booking.com, Expedia, Direct Website
![Project Canvas](docs/images/EDA1.png)
* Key Insights ที่ได้จากกราฟ
    * โรงแรมพึ่งพา OTA สูงมาก
         * Booking.com, Expedia และ Agoda เป็นช่องทางที่สร้างยอดจองส่วนใหญ่ของโรงแรม แสดงให้เห็นว่า OTA เป็นเครื่องมือหลักในการสร้าง Demand และเพิ่ม Occupancy
    * Direct Channel ยังมีศักยภาพในการเติบโต
         * แม้ Direct Website จะมีจำนวนการจองอยู่ในระดับกลาง แต่ยังต่ำกว่า OTA มาก หากโรงแรมสามารถเพิ่มสัดส่วนการจองผ่าน Direct ได้ จะช่วยลดต้นทุนค่าคอมมิชชั่นและเพิ่มกำไรสุทธิ
    * Corporate Channel ช่วยสร้าง Demand ที่ค่อนข้างเสถียร
         * การจองผ่าน GDS และ Corporate Agent มีปริมาณการจองในระดับปานกลาง ซึ่งมักเป็นลูกค้าธุรกิจที่มีการเดินทางสม่ำเสมอ และช่วยเติม occupancy ในช่วงวันธรรมดา
    * Walk-in มีบทบาทค่อนข้างจำกัด
         * จำนวนการจองจากช่องทาง Walk-in อยู่ในระดับต่ำ แสดงให้เห็นว่าลูกค้าส่วนใหญ่เปลี่ยนพฤติกรรมไปสู่การจองออนไลน์มากขึ้น

## 2. วิเคราะห์ Gross Revenue vs Net Revenue ตามช่องทาง 
* จุดประสงค์ เพื่อเปรียบเทียบรายได้ก่อนหักค่าคอมมิชชันและหลังหักค่าคอมมิชชัน ว่าช่องทางใดเสียต้นทุนมาก 
* เหตุผลที่เลือกใช้ : Clustered Bar Chart สามารถเปรียบเทียบ 2 ค่าในหมวดเดียวกันได้ชัดเจน เช่น Gross Revenue และ Net Revenue ของแต่ละ Channel 
![Project Canvas](docs/images/EDA2.png)
* Key Insights ที่ได้จากกราฟ
    * OTA สร้างรายได้รวมสูงที่สุด แต่มีต้นทุนสูง
         * ช่องทาง OTA มี Gross Revenue สูงที่สุด เมื่อเทียบกับทุกช่องทาง แสดงว่า OTA เป็นแหล่งหลักในการสร้างยอดขายของโรงแรม อย่างไรก็ตาม Net Revenue ของ OTA ลดลงจาก Gross Revenue อย่างชัดเจน สะท้อนว่ามี ค่าคอมมิชชั่นหรือค่าธรรมเนียมสูง ทำให้กำไรสุทธิลดลง
    * Direct Channel มีประสิทธิภาพด้านกำไรสูง
         * ในช่องทาง Direct รายได้ Gross และ Net ใกล้เคียงกันมาก แสดงว่าต้นทุนในการได้ลูกค้าผ่านช่องทางนี้ต่ำกว่าช่องทางอื่น ทำให้ Direct Channel เป็นช่องทางที่มี ประสิทธิภาพในการสร้างกำไรสูง
    * Corporate และ Wholesale มีรายได้ระดับปานกลางและต้นทุนต่ำ
         * ช่องทาง Corporate และ Wholesale มีรายได้รวมไม่สูงเท่า OTA แต่ความแตกต่างระหว่าง Gross และ Net ค่อนข้างน้อย แสดงว่าช่องทางเหล่านี้มีต้นทุน distribution ต่ำ และสามารถสร้าง Net Revenue ได้ค่อนข้างดีเมื่อเทียบกับยอดขาย

## 3. วิเคราะห์ค่า Commission Cost ตาม Channel 
* จุดประสงค์ เพื่อหาว่าช่องทางใดมีต้นทุนค่าคอมมิชชันสูงที่สุด 
* เหตุผลที่เลือกใช้ : Horizontal Bar Chart เหมาะกับการจัดอันดับจากมากไปน้อย ทำให้อ่านชื่อช่องทางได้ง่าย
![Project Canvas](docs/images/EDA3.png)  
* Key Insights ที่ได้จากกราฟ
    * OTA เป็นแหล่งต้นทุนค่าคอมมิชชั่นหลักของโรงแรม
         * จากกราฟพบว่า Booking.com มีค่า Commission Cost สูงที่สุด (~870K) รองลงมาคือ Expedia (~650K) และ Agoda (~490K) แสดงให้เห็นว่า OTA เป็นช่องทางที่สร้างต้นทุนการขายสูงที่สุด เนื่องจากต้องจ่ายค่าคอมมิชชั่นให้กับแพลตฟอร์มในอัตราที่สูง
    * Direct Channel มีต้นทุนต่ำมากเมื่อเทียบกับ OTA
         * การจองผ่าน Direct Website มีค่า Commission Cost ต่ำมาก (~40K) และ Walk-in แทบไม่มีต้นทุนค่าคอมมิชชั่น สะท้อนว่าช่องทาง Direct เป็นช่องทางที่ช่วยเพิ่มกำไรสุทธิให้กับโรงแรมได้มากกว่า
    * Corporate Channel มีต้นทุนระดับปานกลาง
         * ช่องทาง GDS มีค่า Commission Cost ประมาณ ~110K ซึ่งต่ำกว่า OTA มาก แต่ยังมีต้นทุนมากกว่าการจองตรง แสดงว่าช่องทาง Corporate ยังมีค่าธรรมเนียมบางส่วน แต่ถือว่ามีต้นทุนที่สมเหตุสมผลเมื่อเทียบกับปริมาณการจอง

## 4. วิเคราะห์ Net ADR ตามช่องทางการจอง
* จุดประสงค์ เพื่อวัดรายได้เฉลี่ยต่อห้องหลังหักค่าคอมมิชชัน ว่าช่องทางใดคุ้มค่าที่สุด 
* เหตุผลที่เลือกใช้ : Column Chart เหมาะกับการเปรียบเทียบค่าเฉลี่ยระหว่างแต่ละช่องทาง
![Project Canvas](docs/images/EDA4.png)  
* Key Insights ที่ได้จากกราฟ
    * Direct Channel สร้าง Net ADR สูงที่สุด
         * ช่องทาง Direct มี Net ADR สูงที่สุด (~550) แสดงว่าการจองตรงผ่านเว็บไซต์ของโรงแรมสามารถรักษาราคาห้องพักและกำไรสุทธิได้ดีที่สุด เนื่องจากไม่มีค่าคอมมิชชั่นจากตัวกลาง
    * OTA มี Net ADR ต่ำกว่า Direct
         * แม้ OTA จะเป็นช่องทางที่สร้างยอดจองจำนวนมาก แต่ Net ADR อยู่ที่ประมาณ ~420 ซึ่งต่ำกว่า Direct อย่างชัดเจน สะท้อนผลกระทบจากค่าคอมมิชชั่นของแพลตฟอร์ม
    * Corporate มี Net ADR ใกล้เคียง OTA
         * ช่องทาง Corporate (~415) มี Net ADR ใกล้เคียงกับ OTA แต่มีข้อดีคือมักมีต้นทุน distribution ต่ำกว่า ทำให้เป็นช่องทางที่สามารถสร้างรายได้สุทธิได้ค่อนข้างดี
    * Wholesale มี Net ADR ต่ำที่สุด
         * ช่องทาง Wholesale (~350) มี Net ADR ต่ำที่สุด เนื่องจากเป็นการขายห้องพักในราคาที่ต่ำกว่าปกติ (Net Rate) เพื่อให้ตัวแทนหรือผู้จัดจำหน่ายนำไปขายต่อ

## 5. วิเคราะห์ Net RevPAR รายวัน (Weekday vs Weekend) 
* จุดประสงค์ เพื่อดูว่าช่วงวันธรรมดาและวันหยุด ช่องทางใดสร้างกำไรต่อห้องดีที่สุด 
* เหตุผลที่เลือกใช้ : Line Chart เหมาะกับข้อมูลตามช่วงเวลา และช่วยมองแนวโน้มรายวัน
![Project Canvas](docs/images/EDA5.png)  
* Key Insights ที่ได้จากกราฟ
    * OTA สร้าง Net RevPAR สูงที่สุดโดยรวม
         * จากกราฟพบว่า OTA มีค่า Net RevPAR สูงกว่าช่องทางอื่นอย่างชัดเจน โดยอยู่ในช่วงประมาณ 120 - 270 ต่อวัน ซึ่งสะท้อนว่าแม้จะมีค่าคอมมิชชั่น แต่ OTA ยังเป็นช่องทางที่สร้างรายได้สุทธิต่อห้องสูงที่สุด เนื่องจากมีอัตราการจองและความต้องการสูง
    * Direct Channel มี Net RevPAR ระดับปานกลางแต่ค่อนข้างเสถียร
         * ช่องทาง Direct มี Net RevPAR อยู่ประมาณ 40 - 100 ต่อวัน และมีความผันผวนน้อยกว่า OTA แสดงให้เห็นว่าการจองตรงช่วยสร้างรายได้ที่ค่อนข้างสม่ำเสมอ
    * Corporate และ Wholesale มี Net RevPAR ต่ำกว่า
         * ช่องทาง Corporate และ Wholesale มี Net RevPAR ต่ำกว่าช่องทางอื่น โดยส่วนใหญ่ต่ำกว่า 40 ต่อวัน ซึ่งสะท้อนว่าช่องทางเหล่านี้มักใช้ราคาห้องที่ต่ำกว่าเพื่อดึงลูกค้ากลุ่มองค์กรหรือเพิ่ม occupancy
    * Weekend มีแนวโน้มสร้างรายได้สูงกว่าในบางช่องทาง
         * ในบางช่วงของเดือน ค่า Net RevPAR ของ OTA และ Direct เพิ่มขึ้นในช่วง Weekend สะท้อนว่าความต้องการท่องเที่ยวในวันหยุดช่วยเพิ่มรายได้ต่อห้อง โดยเฉพาะผ่าน OTA

## 6. วิเคราะห์ COA% (Cost of Acquisition) ตามช่องทาง 
* จุดประสงค์ เพื่อวัดประสิทธิภาพต้นทุนการได้มาของลูกค้า ว่าช่องทางใดใช้ต้นทุนสูงเกินไป 
* เหตุผลที่เลือกใช้ : Bar Chart เปรียบเทียบต้นทุน acquisition ระหว่างหลายช่องทางได้ชัดเจน
![Project Canvas](docs/images/EDA6.png)
* Key Insights ที่ได้จากกราฟ
    * OTA มี COA% สูงที่สุดอย่างชัดเจน
         * ช่องทาง OTA มีค่า COA% สูงถึงประมาณ 23.29% ซึ่งสูงกว่าช่องทางอื่นมาก แสดงให้เห็นว่าการขายผ่าน OTA ต้องเสียค่าคอมมิชชั่นหรือค่าธรรมเนียมจำนวนมาก ทำให้ต้นทุนในการได้ลูกค้าสูงที่สุด
    * Direct และ Corporate มีต้นทุนใกล้เคียงกันและต่ำกว่า OTA มาก
         * ช่องทาง Corporate มี COA% ประมาณ 7.63% และ Direct ประมาณ 6.60% ซึ่งถือว่าต่ำกว่า OTA อย่างมาก แสดงว่าช่องทางเหล่านี้มีประสิทธิภาพในการสร้างกำไรสุทธิดีกว่า
    * Wholesale แทบไม่มีต้นทุน Acquisition
         * ช่องทาง Wholesale มี COA% ใกล้ 0% เนื่องจากเป็นรูปแบบการขายแบบ Net Rate ที่ไม่มีค่าคอมมิชชั่นเพิ่มเติม ทำให้ต้นทุนการได้ลูกค้าต่ำมาก

## 7. วิเคราะห์ Seasonality (High / Low / Shoulder Season)
* จุดประสงค์ เพื่อดูว่าฤดูกาลมีผลต่อรายได้สุทธิและสัดส่วนการจองจาก OTA หรือไม่ 
* เหตุผลที่เลือกใช้ : Stacked Column Chart แสดงทั้งยอดรวมและสัดส่วน channel ในแต่ละ season ได้พร้อมกัน
![Project Canvas](docs/images/EDA7.png)
* Key Insights ที่ได้จากกราฟ
    * High Season สร้างรายได้สูงที่สุดอย่างชัดเจน
         * High Season มี Net Room Revenue รวมสูงที่สุด (~9.6M) ซึ่งมากกว่าช่วง Shoulder Season และ Low Season อย่างมาก แสดงให้เห็นว่าความต้องการเข้าพักในช่วงนี้สูงที่สุดและเป็นช่วงสำคัญในการสร้างรายได้ของโรงแรม
    * OTA เป็นช่องทางหลักในการสร้างรายได้ทุกฤดูกาล
         * ในทุก Season ส่วนที่ใหญ่ที่สุดของกราฟมาจาก OTA โดยเฉพาะใน High Season ที่สร้างรายได้มากกว่า 5.8M แสดงว่า OTA เป็นช่องทางสำคัญในการดึงลูกค้าและสร้าง Demand ให้กับโรงแรม
    * รายได้ลดลงอย่างมากใน Low Season
         * ใน Low Season รายได้รวมลดลงอย่างชัดเจน (~1.3M) เมื่อเทียบกับ High Season แสดงให้เห็นถึงผลกระทบของ Seasonality ต่อธุรกิจโรงแรม และสะท้อนว่าความต้องการเข้าพักลดลงในช่วงนี้
    * Direct และ Corporate ช่วยกระจายรายได้ในทุก Season
         * แม้ OTA จะเป็นช่องทางหลัก แต่ Direct และ Corporate ยังมีบทบาทในการสร้างรายได้ในทุกฤดูกาล และช่วยกระจายความเสี่ยงจากการพึ่งพา OTA เพียงช่องทางเดียว

## 8. วิเคราะห์ยอดจองแต่ละวันในแต่ละช่องทาง
* จุดประสงค์ เพื่อดูว่าแต่ละช่องมีความนิยมการจองในวันไหน เพื่อหารูปแบบในการเพิ่มโปรโมชันให้เหมาะสม แต่ละช่องทาง
* เหตุผลที่เลือกใช้ : Clustered Bar Chart แสดงให้เห็นชัดเจนถึงยอดจองของแต่ละช่องทางในแต่ละวัน โดยใช้วันในสัปดาห์แทนการใช้ weekend/weekday เพราะการใช้ weekend/weekday จะทำให้รวม weekday เข้าด้วยกันและทำให้ยอดสูงกว่า weekend
![Project Canvas](docs/images/EDA8.png)
* Key Insights ที่ได้จากกราฟ
    * OTA เป็นช่องทางที่สร้างยอดจองสูงที่สุดในทุกวันของสัปดาห์
         * จากกราฟพบว่า OTA มีจำนวนการจองสูงที่สุดอย่างชัดเจน โดยเฉลี่ยมากกว่า 3,000 bookings ต่อวัน ซึ่งสูงกว่าช่องทางอื่นมาก แสดงว่าโรงแรมพึ่งพา OTA อย่างมากในการสร้าง Demand
    * Direct Channel เป็นช่องทางรองที่สร้างยอดจองค่อนข้างสูง
         * ช่องทาง Direct มีจำนวนการจองประมาณ 900–1,050 bookings ต่อวัน ซึ่งถือว่าสูงเป็นอันดับสอง รองจาก OTA และแสดงว่าการจองผ่านเว็บไซต์ของโรงแรมยังมีบทบาทสำคัญ
    * Corporate และ Wholesale มีจำนวนการจองต่ำกว่า
         * ช่องทาง Corporate (~450–620 bookings) และ Wholesale (~380–650 bookings) มีปริมาณการจองน้อยกว่า Direct และ OTA แสดงว่าช่องทางเหล่านี้มีบทบาทรองในการสร้าง volume
    * รูปแบบการจองแตกต่างกันตามประเภทลูกค้า
         * ยอดจองของ Direct และ OTA มีแนวโน้มเพิ่มขึ้นในช่วงวันเสาร์–อาทิตย์ สะท้อนพฤติกรรมการท่องเที่ยวในวันหยุด ขณะที่ Corporate Booking สูงในช่วงวันทำงาน ซึ่งสอดคล้องกับการเดินทางเพื่อธุรกิจ. 

## 9. สัดส่วน Booking Status แยกตาม Channel 
* จุดประสงค์ เพื่อดูคุณภาพของ booking ในแต่ละช่องทาง
* เหตุผลที่เลือกใช้ : Pie Chart เพื่อให้เห็นสัดส่วนคุณภาพของแต่ละช่องแบบ % เพื่อดูว่าช่องทางไหนมีโอกาสโดนยกเลิกมากที่สุด
![Project Canvas](docs/images/EDA9.png)
* Key Insights ที่ได้จากกราฟ
    * อัตราการเข้าพักจริง (Checked-Out) ใกล้เคียงกันในทุกช่องทาง
         * ทุกช่องทางมีสัดส่วน Checked-Out ประมาณ 52–55% แสดงว่าการจองส่วนใหญ่สามารถเปลี่ยนเป็นการเข้าพักจริงได้ และคุณภาพของ booking โดยรวมค่อนข้างใกล้เคียงกัน
    * OTA มีอัตราการยกเลิกต่ำกว่าช่องทางอื่นเล็กน้อย
         * OTA มี Cancellation Rate ประมาณ 9.42% ซึ่งต่ำกว่า Direct (~10.29%) และ Corporate (~10.25%) เล็กน้อย แสดงว่าการจองผ่าน OTA มีความเสี่ยงในการยกเลิกไม่สูงอย่างที่คาด
    * Wholesale มีอัตราการยกเลิกต่ำที่สุด
         * ช่องทาง Wholesale มี Cancellation Rate ต่ำที่สุด (~8.68%) สะท้อนว่าการจองผ่านตัวแทนหรือสัญญาขายแบบ Net Rate มักมีความแน่นอนมากกว่า
...
* จากการวิเคราะห์ Booking Volume พบว่า OTA เป็นช่องทางที่สร้างยอดจองสูงที่สุด โดยมียอดรวมมากกว่า 9,000 bookings ซึ่งสูงกว่าช่องทาง Direct, Corporate และ Wholesale อย่างชัดเจน โดยเฉพาะ Booking.com, Expedia และ Agoda ที่เป็นแหล่ง demand หลักของโรงแรม
...
* จากการวิเคราะห์ COA% พบว่า OTA มีต้นทุนการได้ลูกค้าสูงที่สุด โดยมี COA% เท่ากับ 23.29% ขณะที่ Corporate มีเพียง 7.63%, Direct 6.60%,  และ Wholesale ใกล้เคียง 0%
...
* จากการวิเคราะห์ Rate Code พบว่า Rack Rate มี Net RevPAR สูงที่สุดที่ 140.6 รองลงมาคือ Non-Refundable ที่ 79.9 ขณะที่ Seasonal Promo มี Net RevPAR 38.1 และ AAA Discount มี Net RevPAR เพียง 36.6 และ 36.63 ตามลำดับ ส่วน Member Direct Rate มี Net RevPAR ต่ำที่สุดที่ 16.8
